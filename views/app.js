...
export const winner = new Winner(canvas.width, canvas.height);
...
function removeplayer() {
  for (let i = 0; i < players.length; i++) {
    try {
      if (
        players[i] &&
        players[i].playerY + players[i].playerSize >= canvas.height
      ) {
    ...
    try {
      for (let j = 0; j < projectiles.length; j++) {
        const distance = Math.sqrt(
          (projectiles[j].x - players[i].playerX) ** 2 +
            (projectiles[j].y - players[i].playerY) ** 2
        );
